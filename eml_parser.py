    ...
    def eml_parsing(self):
        try:
            ...
            if len(self.listOfFiles) > 3:
                #result = corePool.map(self.eml_file_handling, (0,1,2,3))
                #result = corePool.map(testfunc, range(4))
                self.eml_file_handling()
            ...
            self.warning_message("Specify the path to the eml file.")

    def eml_file_handling(self):

        for thefile in self.listOfFiles:

            # db 저장할 경로 입력 받기
            try:
                db_path = str(self.savepath) + "\\"
                if not os.path.isfile(db_path + "result.db"):
                    result_db = res_db.sqlite_db(db_path)
                else:
                    pass
            except AttributeError:
